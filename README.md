...
> **Why this repository has no source code.** ApplyWorth is a live product with real users, so the
> application repository is private. This is a public case study covering architecture, schema, and
...
exchange programmes, and relocation pathways) collected automatically and kept current.
...
│ pages         │         │ to Document Vault│        │ report, digest   │
└───────────────┘         └──────────────────┘        └──────────────────┘
```
...
is scoped to `auth.uid()`. Migrations are numbered and append-only, so an existing migration is
...
- **Chose `slug` as the dedup key with `ignoreDuplicates: true`, over upserting on a content hash.**
  ...
  formatting. A content hash treats those as distinct rows, whereas a normalised slug collapses
  them. Using `ignoreDuplicates` rather than overwrite means a row a human has since corrected is
  never clobbered by a later scrape of the same posting. Dedup runs twice, in memory before the
  batch and again at the database via the unique constraint, because a single sync run can pull the
...
- **Chose lazy client construction over module-level instantiation.**
  ...
  user gets Next.js's default HTML error page instead of a structured JSON error. On a streaming
  endpoint that means an HTML document arriving where the client expects an SSE stream.
...
- **Chose to flush a `: warmup` SSE comment before the first real chunk.**
  ...
  recoverable in-stream error rather than a dead connection. Relatedly, only `Content-Type` and
  `Cache-Control` are set on SSE responses, because `Connection: keep-alive` is forbidden under
  HTTP/2 and was the root cause of a long-running class of intermittent 500s on Vercel.
...
- **Per-source isolation.** Each adapter runs inside `runAdapter()`. One source timing out, changing
  its schema, or rate-limiting the crawler degrades that source only, and the sync completes with
  the remaining 24.
...
- **Cost control.** Prompt caching on the profile and opportunity prefix, Haiku for non-production
  paths, and per-user sliding-window rate limits backed by Redis.
...
- **CV ingestion.** PDF and DOCX parsing (`unpdf`, `mammoth`) into the structured profile, with
  export back out to PDF and DOCX.
